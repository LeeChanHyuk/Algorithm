...
def bfs(queue, vias, keys, col, row, paper):
	unopened_door = []
...
for case in range(test_case):
	...
	for i in range(col):
		if vias[0][i] == 0:
			if board[0][i] == '.' or board[0][i].upper() in keys:
				queue.append([0, i])
				vias[0][i] = 1
			elif board[0][i] == '$':
				queue.append([0, i])
				...
				vias[0][i] = 1
		if vias[row-1][i] == 0:
			if board[row-1][i] == '.' or board[row-1][i].upper() in keys:
	...
	for i in range(row):
		if vias[i][0] == 0:
			if board[i][0] == '.' or board[i][0].upper() in keys:
				queue.append([i, 0])
				vias[i][0] = 1
			elif board[i][0] == '$':
				queue.append([i, 0])
				...
				vias[i][0] = 1
		if vias[i][col-1] == 0:
			if board[i][col-1] == '.' or board[i][col-1].upper() in keys:
	...
	paper_num = bfs(queue, vias, keys, col, row, paper_num)
	print(paper_num)
